...
function is_course_model() {
    if ($("#is_course_model").is(':checked')) {
        $('#is_course_model').attr('value', '1');
    } else {
        $('#is_course_model').attr('value', '0');
    }
}
...
jQuery(document).ready(function () {
  ...
  $(".f1 .f1-buttons .btn-next").on("click", function () {
    ...
    // fields validation
    parent_fieldset
      .find('input[type="checkbox"]#flexCheckDefault')
      .each(function () {
        if ($(this).val() === "") {
    ...
    if (next_step) {
      parent_fieldset.fadeOut(400, function () {
        ...
        var is_course_model = $("#is_course_model").val();
        
        // alert(is_course_model);
        ...
        $.ajax({
          ...
          data: {
            course_id: course_id,
            terms: terms,
            sale_benefits: sale_benefits,
            subscribe_benfits: subcrib_benefits,
            is_course_model: is_course_model,
  ...
  $(".f1  .f2-buttons .btn-next").on("click", function () {
    ...
    if (edit_type) {
      parent_fieldset
        .find(
          "#old_hover_thum,#old_minithum,#old_thumbnail,#course_level,#courseCatagory,#courseTitle"
        )
        .each(function () {
          // if ($(this).val() === "") {
          //   console.log(this);
          ...
          if ($(this).val() === "") {
            ...
            // $(this).addClass('input-error');
            if($("#courseTitle").val() === ""){
              $("#courseTitle").addClass("input-error");
              next_step = false;
              $(this).focus();
            }
            if($("#courseCatagory").val() === ""){
              $("#courseCatagory").addClass("input-error");
              next_step = false;
              $(this).focus();
            }
            if($("#course_level").val() === ""){
              $("#course_level").addClass("input-error");
              next_step = false;
              $(this).focus();
            }
            if($("#imgupload").val() === ""){
              $("#imgupload").addClass("input-error");
              next_step = false;
              $(this).focus();
            }
            if($("#hover_thumbnail").val() === ""){
              $("#hover_thumbnail").addClass("input-error");
              next_step = false;
              $(this).focus();
            }
            if($("#cover_thumbnail").val() === ""){
              $("#cover_thumbnail").addClass("input-error");
              next_step = false;
              $(this).focus();
            }
      ...
      parent_fieldset
        .find(
          "#cover_thumbnail,#hover_thumbnail,#imgupload,#imgupload2,#formFile2,#course_level,#courseCatagory,#courseTitle"
        )
        .each(function () {
          if ($(this).val() === "") {
            console.log(this);
    ...
    if (next_step) {
      parent_fieldset.fadeOut(400, function () {
        ...
        var minitumbnail_croped_url = $("#thumbnail_croped_url").val();
        var minitumbnail_filename = $("#thumbnail_original_filename").val();
        var old_mini_thumbnail_name = $("#old_mini_thumbnail_name").val();
        // alert(minitumbnail_croped_url);
        var hover_croped_url = $("#thumbnailhover_croped_url").val();
        var hover_filename = $("#hover_original_filename").val();
        var old_hover_thumbnail_name = $("#old_hover_thumbnail_name").val();

        var cover_croped_url = $("#thumbnailcover_croped_url").val();
        var cover_filename = $("#cover_original_filename").val();
        var old_cover_thumbnail_name = $("#old_cover_thumbnail_name").val();

        fd.append("url", $("#url").val());
        fd.append("passing_grade", $("#passing_grade").val());
        // fd.append("thumbnail", $("#formFile2")[0].files[0]); //cover
        fd.append("thumbnail", cover_croped_url); //cover
        fd.append("cover_original_filename", cover_filename); //cover
        fd.append("old_cover_thumbnail_name", old_cover_thumbnail_name); //cover

        // fd.append("mini_thumbnail", $("#imgupload")[0].files[0]) hover image;

        fd.append("mini_thumbnail", minitumbnail_croped_url); // thumbnail image
        fd.append("thumbnail_original_filename", minitumbnail_filename); // thumbnail image
        fd.append("old_mini_thumbnail_name", old_mini_thumbnail_name); // thumbnail image

        // fd.append("thumbnail_hover", $("#imgupload2")[0].files[0]);
        fd.append("thumbnail_hover", hover_croped_url); //hover thumbnail
        fd.append("hover_original_filename", hover_filename); //hover thumbnail;
        fd.append("old_hover_thumbnail_name", old_hover_thumbnail_name); //hover thumbnail;
        ...
        for (var i = 0; i < gainskill.length; i++) {
         ...
         var language = JSON.stringify($('[name="language[]"]').val());
