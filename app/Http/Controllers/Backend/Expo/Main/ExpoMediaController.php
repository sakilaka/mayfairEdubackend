...
class ExpoMediaController extends Controller
{
    ...
    public function expo_gallery_page_update(Request $request, $expo_id)
    {
        try {
            ...
            $allGalleries = [];
            $oldGalleries = json_decode($expo->gallery, true) ?? []; // Decode old galleries
            ...
            foreach ($oldGalleries as $oldGalleryKey => $oldGallery) {
                if (!isset($allGalleries[$oldGalleryKey])) {
                    // Gallery has been removed; delete its images
                    foreach ($oldGallery['images'] as $imageUrl) {
            ...
            $expo->gallery = json_encode($allGalleries); // Save updated galleries
            $expo->save();
