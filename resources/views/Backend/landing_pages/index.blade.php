                                ...
                                <tbody>
                                    @foreach ($pages as $page)
                                        <tr role="row" class="odd">
                                            ...
                                            <td>
                                                {{ Illuminate\Support\Str::limit($page->title, 50, '...') }}
                                            </td>
                                            <td>
                                                <a href="{{ route('landing_page.show_page', ['slug' => $page->slug]) }}"
                                                    target="_blank" class="text-primary">
                                                    {{ Illuminate\Support\Str::limit(route('landing_page.show_page', ['slug' => $page->slug]), 50, '...') }}
                                                </a>
                                            </td>
                                            <td class="text-center">
                                                @if ($page->status == 1)
                                                    <a
                                                        href="{{ route('admin.landing_page.change_status', $page->id) }}">
                        ...
                        <div class="modal-content">
                            <div class="modal-body text-center">
                                ...
                                <div class="m-t-20 flex">
                                    <form action="{{ route('admin.landing_page.delete') }}" method="POST"
                                        id="deleteForm">
                                        @csrf
